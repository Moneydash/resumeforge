...
const ClassicTemplate: React.FC<TemplateProps> = ({ data, parseArrayField }) => (
  <div className="max-w-4xl mx-auto bg-white print:bg-white text-slate-900 print:text-black font-serif text-base leading-relaxed print:text-sm px-8 pt-12 space-y-8">
    {/* Header */}
    <header className="text-center border-b border-slate-300 pb-5">
      ...
      {data.personal.headline && (
        <h2 className="text-xl text-slate-600 italic tracking-wide mb-4">
          {data.personal.headline}
        </h2>
      )}
        ...
        {data.experience.map((exp, index) => (
          <div key={index} className="mb-5">
            <div className="flex justify-between items-start">
              <h4 className="text-lg font-medium text-slate-800">{exp.title}</h4>
        ...
        {data.education.map((edu, index) => (
          <div key={index} className="mb-6">
            <div className="flex justify-between mb-1">
              <h3 className="text-xl font-medium text-slate-800">{edu.degree}</h3>
              <span className="text-sm text-slate-600 bg-slate-100 px-2 py-1 rounded-full">
    ...
    {data.projects?.length > 0 && (
      <Section title="Projects">
        {data.projects.map((project, index) => (
          <div key={index} className="mb-6">
            <h3 className="text-lg font-medium text-slate-800 mb-1">{project.title}</h3>
    ...
    {data.certifications?.length > 0 && (
      <Section title="Certifications">
        {data.certifications.map((cert, index) => (
          <div key={index} className="flex justify-between items-center border-b border-slate-200 py-3">
            <div>
              <p className="text-lg font-medium text-slate-800">{cert.name}</p>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => (
  <section>
    <h2 className="text-xl font-semibold uppercase text-slate-800 border-b pb-1 mb-1">
      {title}
    </h2>
    {children}
  </section>
...
const SkillList: React.FC<{ title: string; skills: string[] }> = ({ title, skills }) => (
  <div>
    <h3 className="text-lg font-medium text-slate-700 mb-3">{title}</h3>
